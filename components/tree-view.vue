<template>
  ...
  <el-drawer title="Filtra per Agente/Agenzia" :visible.sync="value" :direction="'rtl'" :before-close="vModel">
    <div style="padding: 0px 20px">
      <el-autocomplete clearable @clear="search.length === 0 ? resetTree() : null" v-model="search" :fetch-suggestions="querySearchAsync" style="width:100%" value-key="nominativo" placeholder="Ricerca Agente" @select="handleSelect"></el-autocomplete>
    ...
    async getAgenti() {
      ...
      this.$axios
        ...
        .then(
          ({ data }) => {
            this.agenti = data;
            this.defaultAgenti = JSON.parse(JSON.stringify(data));
