  ...
  methods: {
    ...
    handleStartMensile(){
      if(!this.formMensile.anno || !this.formMensile.anno){
        ...
        this.$axios.post('/api/rcu-ee/cc/download-file/', {anno:this.formMensile.anno,mese:this.formMensile.mese},{
    ...
    handleStart(){
      if(!this.formAnnuale.anno){
        ...
        this.$axios.post('/api/rcu-ee/cc/esportazione/', {anno:this.formAnnuale.anno},{
          headers:{'Secret-Key' : this.$store.getters['authToken']},
          responseType: 'arraybuffer',
        })
        .then(function(response){
