...
<script>
import esporta from '@/components/rcu-gas/cc/esporta.vue'
import importa from '@/components/rcu-gas/cc/importa.vue'
import check from '@/components/rcu-gas/cc/check.vue'
...
export default {
  ...
  methods: {
    downloadCSV(id){
      ...
      this.$axios.post('/api/rcu-gas/cc/download-file', {id},{
     ...
     deleteFatt(id){
       this.$confirm('Ricorda che verrano eliminati tutti i record relativi questa importazione.', 'Sei sicuro di voler cancellare questo elemento?', {
          ...
          this.$axios.post('/api/rcu-gas/cc/delete-fatturazione', 
            {id},
            {
              headers:{'Secret-Key' : this.$store.getters['authToken']
            },
     ...
     fetchFolderContent() {
      this.$axios.get('/api/rcu-gas/cc/history', {
        headers:{
     ...
     fetchStatusContent() {
      this.$axios.get('/api/rcu-gas/cc/status', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        }
      }).then(
