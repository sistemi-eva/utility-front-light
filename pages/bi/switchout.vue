<template>
  <div>
    ...
    <el-row :gutter="20">
        <el-col :span="14">
            <span style="font-size:20pt; font-weight:600">Clienti Out</span>
        </el-col>
    </el-row>
    ...
    <div class="main" v-if="query1.length > 0">
      ...
      <div v-show="!isLoading" id="chart">
        <div class="el-row" style="padding-top: 30px">
          <div class="el-col el-col-12" style="padding-right: 10px;">
            <div class="el-card box-card is-always-shadow">
              <div class="el-card__header"><div class="clearfix"><span>Metodo di Pagamento {{ meseSelezionatoPrec  }} {{ annoSelezionato }}</span></div></div>
              <div class="el-card__body">
                <el-row>
                  <el-col :span="24">
                    <apexchart ref="mychart3" type="pie" height="350" :options="graph3.chartOptions" :series="graph3.series"></apexchart>
                  </el-col>
          ...
          <div class="el-col el-col-12" style="padding-left: 10px;padding-right: 10px;">
            <div class="el-card box-card is-always-shadow">
              <div class="el-card__header"><div class="clearfix"><span>Metodo di Pagamento {{ meseSelezionatoPrec2  }} {{ annoSelezionato }}</span></div></div>
              <div class="el-card__body">
                <el-row>
                  <el-col :span="24">
                    <apexchart ref="mychart4" type="pie" height="350" :options="graph4.chartOptions" :series="graph4.series"></apexchart>
                  </el-col>
                </el-row>
              </div>
            </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        graph3:{
          series: this.graph3Serie1,
          chartOptions: {
            chart: {
              width: 380,
              type: 'pie',
            },
            labels: ['Bollettino', 'Rid'],
            plotOptions: {
              pie: {
                startAngle: -90,
                endAngle: 270
            ...
            tooltip: { 
              ...
              custom: function ({ series, seriesIndex, dataPointIndex, w }) { 
                let total = 0; for (let x of series) { total += x; } let selected = 
                series[seriesIndex] 
        ...
        metodoDiPagamentoBollettino: 0,
        metodoDiPagamentoRid:0,

        metodoDiPagamentoBollettinoPrec : 0,
        metodoDiPagamentoRidPrec: 0,
    ...
    methods: {
      ...
      async updateAll(){
        ...
        this.$axios.post('/api/reportistica/get-switchout', { mese: this.meseNumerico[this.meseSelezionato], anno: this.annoSelezionato, azienda: this.aziendaSelezionata },{
        ...
        .then(function(response){
          ...
          for (var i in self.query1){
            if( self.query1[i]["Mese Emissione"] == mprec ){
              self.tableData[ self.query1[i].STATORIGA ] = {"GAS": 0, "EE": 0};   

              if( self.query1[i]["MetodoPagamento"] == "RID / SDD" ){
                self.metodoDiPagamentoRid++;
              }
              else{
                self.metodoDiPagamentoBollettino++;
              }

            }
            if( self.query1[i]["Mese Emissione"] == mprec2 ){
              self.tableData2[ self.query1[i].STATORIGA ] = {"GAS": 0, "EE": 0};   

              if( self.query1[i]["MetodoPagamento"] == "RID / SDD" ){
                self.metodoDiPagamentoRidPrec++;
              }
              else{
                self.metodoDiPagamentoBollettinoPrec++;
              }
          ...
          for (var i in self.query1){
            ...
            if( self.query1[i]["Mese Emissione"] == mprec ){
              if( self.query1[i]["CODUTILITY"]  == "GAS"  ){
                self.tableData[ self.query1[i].STATORIGA ].GAS++;
              }

              if( self.query1[i].CODUTILITY == "EE"){
                self.tableData[ self.query1[i].STATORIGA ].EE++;
              }
            ...
            if( self.query1[i]["Mese Emissione"] == mprec2 ){
              if( self.query1[i]["CODUTILITY"]  == "GAS"  ){
                self.tableData2[ self.query1[i].STATORIGA ].GAS++;
              }

              if( self.query1[i].CODUTILITY == "EE"){
                self.tableData2[ self.query1[i].STATORIGA ].EE++;
              }
          ...
          self.graph3.series = [self.metodoDiPagamentoBollettino, self.metodoDiPagamentoRid];
          
          self.graph4.series = [self.metodoDiPagamentoBollettinoPrec, self.metodoDiPagamentoRidPrec];
