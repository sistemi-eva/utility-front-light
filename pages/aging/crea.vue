<template>
    <div>
      ...
      <el-divider></el-divider>
       <el-alert
        title="Il Gestore File accetta solo file XLSX"
        type="warning"
        description="Se il tuo file è diverso occorre modificare il formato del file in modo che sia possibile gestirlo. N.B L'APPLICATIVO NECESSITA DI DUE RIFERIMENTI all'interno del XLSX ovvero PIVA/CFISC."
        >
      </el-alert>
      <el-divider></el-divider>
      <el-upload
  ...
  data: () => {
   ...
   methods: {
     ...
     uploadFunction(){
      ...
      fd.append("xlsx_file", this.fileList[0]);
     //send call the api to upload files using axios or any other means
       this.$axios.post('/api/aging/start-aging', fd,{
       ...
       .then(function(response){
         const url = window.URL.createObjectURL(new Blob([response.data]));
          ...
          link.href = url;
          link.setAttribute('download', 'aging.xlsx'); //or any other extension
