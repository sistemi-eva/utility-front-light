<template>
  <div>
    <esporta v-if="esportazione" v-model="esportazione"/>
    <fornitori v-if="fornitori" v-model="fornitori" />
    <normalizza-campi  v-if="normalizzazione"  v-model="normalizzazione" />
    <testata-guida v-if="testata" v-model="testata" />
    <importa v-if="importa" v-model="importa" @updateStatus="handleUpdateClick" />
    <div>
      <el-row class="title-container">
        <el-col :span="4">
          <h2><i class="el-icon-s-cooperation"></i> Fatturazione Passiva</h2>
        </el-col>
        <el-col :span="10">
        ...
        <el-col :span="14" style="text-align:right">
          <el-button type="primary" icon="el-icon-document" @click="showEsportazione">Esporta</el-button>
          <el-button type="primary" icon="el-icon-user-solid" @click="showFornitori">Fornitori</el-button>
          <el-button type="primary" icon="el-icon-tickets" @click="showNormalizzazione">Normalizza Campi</el-button>
          <el-button type="primary" icon="el-icon-guide" @click="showTestata">Testata</el-button>
          <el-button  @click="showImporta" icon="el-icon-plus" type="primary">Importa</el-button>
        </el-col>
      </el-row>
...
<script>
import importa from '@/components/fatturazione_passiva/importa.vue'
import fornitori from '@/components/fatturazione_passiva/fornitori.vue'
import normalizzaCampi from '@/components/fatturazione_passiva/normalizzaCampi.vue'
import testataGuida from '@/components/fatturazione_passiva/testataGuida.vue'
import esporta from '@/components/fatturazione_passiva/esporta.vue'

export default {
  components: { importa,fornitori,normalizzaCampi,testataGuida,esporta},
  ...
  computed: {
    ...
    pagedTableData() {
      const temps = this.history.filter(data => !this.search 
       ...
       || data.fornitore.toLowerCase().includes(this.search.toLowerCase())
  ...
  mounted() {
   ...
   methods: {
     downloadCSV(id){
      ...
      this.$axios.post('/api/fatturazione-passiva/download-file', {id},{
     ...
     deleteFatt(id){
       this.$confirm('Ricorda che verrano eliminati tutti i record relativi questa importazione.', 'Sei sicuro di voler cancellare questo elemento?', {
          ...
          this.$axios.post('/api/fatturazione-passiva/delete-fatturazione', 
            {id},
            {
              headers:{'Secret-Key' : this.$store.getters['authToken']
            },
     ...
     fetchFolderContent() {
      this.$axios.get('/api/fatturazione-passiva/history', {
        headers:{
     ...
     fetchStatusContent() {
      this.$axios.get('/api/fatturazione-passiva/status', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        }
      }).then(
    ...
    showTestata(){
     ...
     showEsportazione(){
       this.esportazione = true
     },
     showFornitori(){
       this.fornitori = true
     },
     showNormalizzazione(){
       this.normalizzazione = true
     },
    },
