<template>
<div v-if="tenant === 'ugm'" v-loading="loading">
  <el-row  :gutter="20">
    <el-col :span="8">
      <span style="font-size:30pt; font-weight:600">{{selectedLocations ? selConvertLocation :  "Parco Totale Energia"}}</span>
    </el-col>
     ...
     <el-col :span="9"> 
       ...
       <div style="display:flex; justify-content:space-around">
         <div style="text-align:center">
          <div style="background-color:#eee5c3; width:80px;height:30px;" /><span style="font-size:8.5pt">Min. di 250</span>
         </div>
         <div style="text-align:center">
          <div style="background-color:#decd87; width:80px;height:30px;" /><span style="font-size:8.5pt">Tra 251 e 1000</span>
         </div>
         <div style="text-align:center">
          <div style="background-color:#d2bb5f; width:80px;height:30px;" /><span style="font-size:8pt">Tra 1001 e 5000</span>
         </div>
         <div style="text-align:center">
          <div style="background-color:#806924; width:80px;height:30px;" /><span style="font-size:8pt">Tra 5001 e 10000</span>
         </div>
         <div style="text-align:center">
          <div style="background-color:#805e00; width:80px;height:30px;" /><span style="font-size:7.5pt">Tra 10001 e 25000</span>
         </div>
         <div style="text-align:center">
          <div style="background-color:#4f4116; width:80px;height:30px;" /><span style="font-size:8.5pt">Mag. di 25001</span>
         </div>
...
export default {
  ...
  async mounted() {
    if(this.$store.state.tenant_ee === 'ugm' || this.$cookies.get('tenant_ee') === 'ugm') {
  ...
  methods: {
    esporta(){
      ...
      this.$axios.post('/api/rcu-ee/cc/italy-map-export/', {
    ...
    async getLastImport() {
     ...
     async test() {
      await this.$axios.get('/api/rcu-ee/cc/italy-map', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
