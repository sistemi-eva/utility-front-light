<template>
  <div>
    ...
    <el-row class="title-container">
      <el-col :span="4">
        <h2><i class="el-icon-s-order"></i> RCU CC</h2>
      </el-col>
      <el-col :span="10">
      ...
      <el-col :span="14" style="text-align:right">
        <span v-if="!!status_cache">
          <el-button v-if="status_cache != 'cached_old'" :loading="status_cache == 'in_lavorazione' ? true : false" :disabled="status_cache == 'cached' ? true : false" @click="refreshCache" type="primary" icon="el-icon-refresh">{{ status_cache == "in_lavorazione" ? "Cache in aggiornamento" : "Aggiorna Cache" }}</el-button>
          <el-button v-else :loading="status_cache == 'in_lavorazione' ? true : false" :disabled="status_cache == 'cached' ? true : false" @click="refreshCache" type="danger" icon="el-icon-refresh">Aggiorna cache con ultima versione</el-button>
          <el-button v-if="status_cache != 'in_lavorazione'" @click="deleteCache" type="primary" icon="el-icon-delete">Elimina Cache</el-button>
        </span>
    ...
    <el-alert v-if="showWarning == 'in lavorazione'" type="warning" description="Al momento sono in corso già piu importazioni, attendi la fine per inizarne una nuova" show-icon :closable="false">
      <span slot="title">
    ...
    <el-alert v-else title="Puoi avviare una nuova importazione " type="success" description="Recati nella schermata apposita per iniziare una nuova importazione" show-icon :closable="false">
      <span slot="title">
    ...
    <el-divider></el-divider>
    <el-table :data="pagedTableData" :row-class-name="tableRowClassName" style="width: 100%">
      <el-table-column prop="id" label="ID Importazione" width="130"> </el-table-column>

      <el-table-column prop="note" label="Note Aggiuntive" align="center"> </el-table-column>
      <!-- <el-table-column
          prop="note"
          label="Note"
          align="center"
      ...
      <el-table-column prop="mese" label="Mese" align="center"> </el-table-column>
      <el-table-column prop="anno" label="Anno" align="center"> </el-table-column>
      <el-table-column prop="importati" label="Elementi Importati" align="center"> </el-table-column>
      <el-table-column prop="status" label="Stato Importazione" align="center">
      ...
      <el-table-column align="center" label="Creatore" width="340px">
        <template slot-scope="scope">
          <div class="tooltip">
    ...
    <el-pagination layout="prev, pager, next" :current-page.sync="page" :page-size="pageSize" :total="total" @current-change="setPage"> </el-pagination>
  </div>
</template>

<script>
import esporta from "@/components/rcu-ee/cc/esporta.vue";
import importa from "@/components/rcu-ee/cc/importa.vue";
import check from "@/components/rcu-ee/cc/check.vue";
...
export default {
  ...
  computed: {
    ...
    pagedTableData() {
      const temps = this.history.filter(
        (data) =>
          !this.search ||
          data.id
            .toString()
  ...
  methods: {
    ...
    deleteFatt(id) {
      this.$confirm("Ricorda che verrano eliminati tutti i record relativi questa importazione.", "Sei sicuro di voler cancellare questo elemento?", {
        ...
        .then(() => {
          ...
          this.$axios
            .post(
              "/api/rcu-ee/cc/delete-fatturazione",
              { id },
              {
                headers: { "Secret-Key": this.$store.getters["authToken"] },
              }
    ...
    statusCache() {
      this.$axios
        .get("/api/rcu-ee/cc/status-cache", {
          headers: {
            "Secret-Key": this.$store.getters["authToken"],
          },
        })
    ...
    deleteCache() {
      this.$axios
        .get("/api/rcu-ee/cc/delete-cache", {
          headers: {
            "Secret-Key": this.$store.getters["authToken"],
          },
        })
    ...
    refreshCache() {
      this.$axios
        .get("/api/rcu-ee/cc/refresh-cache", {
          headers: {
            "Secret-Key": this.$store.getters["authToken"],
          },
        })
