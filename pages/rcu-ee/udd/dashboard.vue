<template>
<div v-if="!showError">
  <tabella-info-bonus :type="'rcu-ee/udd'" :mese="mesi.indexOf(meseSelezionato)+1" :anno="annoSelezionato" v-model="dialogTableVisible" />
  <el-row :gutter="20">
    <el-col :span="14">
  ...
  <el-row style="padding-top:30px" :gutter="20">
    <el-col :span="12">
      ...
      <div slot="header" class="clearfix">
        <span>Dati pod e volumi per Reseller</span>
      </div>
 ...
 components: {
  ...
  methods: {
    ...
    async tableBonusInformation() {
      this.$axios.get('/api/rcu-ee/udd/table-bonus-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async chartIncrementoAnnBonusInformation(){
      this.$axios.get('/api/rcu-ee/udd/chart-incremento-bonus-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    esporta(){
      ...
      this.$axios.post('/api/rcu-ee/udd/download-file/', {mese: this.mesi.indexOf(this.meseSelezionato)+1,anno:this.annoSelezionato},{
    ...
    async tableInformation() {
      this.$axios.get('/api/rcu-ee/udd/table-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async sintesiInformation() {
      this.$axios.get('/api/rcu-ee/udd/sintesi-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async chartIncrementoAnnPodInformation(){
      this.$axios.get('/api/rcu-ee/udd/chart-incremento-pod-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async chartIncrementoAnnVolumeInformation(){
      this.$axios.get('/api/rcu-ee/udd/chart-incremento-volume-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
