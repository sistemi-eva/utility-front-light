...
<div v-if="!showError">
  ...
  <el-tabs style="margin-top:30px" type="border-card">
    <el-tab-pane label="Pod"><tabellastatisticheannuali :andamento="false" :tableAnnuale="tablePodAnnuale" /></el-tab-pane>
    <el-tab-pane label="Volume"><tabellastatisticheannuali :andamento="false" :tableAnnuale="tableVolumiAnnuale" /></el-tab-pane>
    <el-tab-pane label="Andamento Relativo Pod"><tabellastatisticheannuali :andamento="true" :tableAnnuale="tableIncPodAnnuale" /></el-tab-pane>
  ...
  <el-row style="padding-top:50px" :gutter="20">
    <el-col :span="24">
      <el-tabs v-model="tabs" @tab-click="changeTab" type="border-card">
        ...
        <el-tab-pane name="pod-3" label="Andamento POD Ultimi 3 anni" ><graficoultimianni :seriesGrafico3Years="seriesGraficoPod3Years"  /></el-tab-pane>
    ...
    <el-col style="padding-top:50px" :span="12">
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          ...
          <span>Pod persi (Data Importazione RCU) in base mensile</span>
          <div style="display:flex;justify-content:center;align-items:center">
            ...
            <div slot="content">
              <h3>Pod Persi mensilmente rispetto al totale dei pod del mese impostato,<br> 
    ...
    <el-col style="padding-top:50px" :span="12">
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <div style="display:flex;align-items:center;justify-content: space-between;">
          <span>Pod persi (Data Attivazione Fornitura) in base mensile</span>
          <div style="display:flex;justify-content:center;align-items:center">
            ...
            <el-tooltip class="item" effect="dark" content="" placement="left-start">
            <div slot="content">
              <h3>Pod Persi mensilmente rispetto alla data attivazione della fornitura del mese impostato,<br> 
              con la possibilità di analizzare dettagliatamente la perdita percentuale</h3>
              </div>
              <i style="padding-left:20px;" class="el-icon-info"></i>
            </el-tooltip>
...
export default {
 ...
 components: {
  ...
  methods: {
    async getLastImport() {
      await this.$axios.get('/api/rcu-ee/udd/last-import', {
        headers:{
    ...
    esporta(){
      ...
      this.$axios.post('/api/rcu-ee/udd/esportazione/', {anno:this.annoSelezionato},{
    ...
    async updateAll(){
      ...
      this.$axios.get('/api/rcu-ee/udd/check-sintesi', {
      headers:{
        'Secret-Key' : this.$store.getters['authToken']
      },
      params: {
    ...
    async chartPod3YearsInformation(){
      this.$axios.get('/api/rcu-ee/udd/chart-pod-3years-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async chartVolumi3YearsInformation(){
      this.$axios.get('/api/rcu-ee/udd/chart-volumi-3years-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async chartInformation(){
      this.$axios.get('/api/rcu-ee/udd/chart-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async chartPodInformation(){
      this.$axios.get('/api/rcu-ee/udd/chart-pod-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async chartPodDispacciamentoInformation(){
      this.$axios.get('/api/rcu-ee/udd/chart-pod-dispacciamento-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async tablePodInformation(){
      this.$axios.get('/api/rcu-ee/udd/table-pod-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async tableIncPodInformation(){
      this.$axios.get('/api/rcu-ee/udd/table-inc-pod-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async tableVolumiInformation(){
      this.$axios.get('/api/rcu-ee/udd/table-volumi-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
    ...
    async tableIncVolumiInformation(){
      this.$axios.get('/api/rcu-ee/udd/table-inc-volumi-information', {
        headers:{
          'Secret-Key' : this.$store.getters['authToken']
        },
        params: {
